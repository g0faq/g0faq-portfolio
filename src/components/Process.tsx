  ...
  return (
    <section className="section">
      ...
      <div className="relative grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        ...
        {processSteps.map((step, index) => <Reveal key={step} delay={index * 0.07}><article className="process-card"><p className="font-display text-5xl font-semibold text-white/10">0{index + 1}</p><span className="process-dot" /><p className="mt-auto pt-12 text-base font-semibold leading-7 text-white">{step}</p><div className="mt-7 h-1 w-full rounded-full bg-premium-gradient" /></article></Reveal>)}
      </div>
    </section>
  );
}

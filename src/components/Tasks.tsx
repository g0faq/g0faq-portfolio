    ...
    <section className="section">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-6">
        {tasks.map(([title, text], index) => {
          ...
          return <Reveal key={title} className={size} delay={index * 0.06}><article className={`task-card h-full ${index === 0 ? "min-h-[310px]" : "min-h-[190px]"}`}><span className="task-number">0{index + 1}</span><div className="icon-tile"><Icon className="h-5 w-5" /></div><h3 className="mt-auto pt-8 font-display text-2xl font-semibold text-white">{title}</h3><p className="mt-3 max-w-xl text-sm leading-6 text-white/[0.65]">{text}</p></article></Reveal>;
        })}
      </div>
    </section>
  );
